...
cal_dens_ODP<-function(y, model,newdata){
...
cal_dens_LN<-function(y, model,tau,data,newdata){
...
cal_mu_LN<-function(model,tau,data,newdata){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
...
##Calculate LN mean for LN GLM
cal_sigma_LN<-function(model,tau,data,newdata){
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
    return(pred_sigma)
}

##Calculate Log-Normal CDF from Log-Normal GLM
cal_CDF_LN<-function(y, model,tau,data,newdata, new_y = F){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
...
cal_dens_GA<-function(y, model,tau,data,newdata){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata, type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
...
cal_mu_GA<-function(model,tau,data,newdata){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    return(pred_mu)
}

##Calculate GA mean for GA GLM
cal_sigma_GA<-function(model,tau,data,newdata){
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
    return(pred_sigma)
}
...
cal_CDF_GA<-function(y, model,tau,data,newdata, new_y = F){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
...
cal_dens_Normal<-function(y, model, data,newdata){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
...
cal_mu_Normal<-function(model, data,newdata){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    return(pred_mu)
}

##Calculate Normal mean for Normal GLM
cal_sigma_Normal<-function(model, data,newdata){
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
    return(pred_sigma)
}

##Calculate Normal CDF from Normal GLM
cal_CDF_Normal<-function(y, model, data,newdata, new_y = F){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
...
cal_dens_ZALN<-function(y, model,data,newdata){
    newdata_nu<-newdata
...
cal_mu_ZALN<-function(model,data,newdata){
    newdata_nu<-newdata
    newdata_nu$dev=as.numeric(as.character(newdata$dev))
    pred_mu<-predict(model,parameter="mu",data=data,newdata=newdata,type="response")
    pred_nu<-predict(model,parameter="xi0",data=data,newdata=newdata_nu,type="response")
...
##Calculate ZALN mean for ZALN GLM
cal_sigma_ZALN<-function(model,data,newdata){
    newdata_nu<-newdata
    newdata_nu$dev=as.numeric(as.character(newdata$dev))
    pred_sigma <- predict(model,parameter="sigma",data=data,newdata=newdata,type="response")
    return(pred_sigma)
...
cal_CDF_ZALN<-function(y, model,data,newdata, new_y = F){
    newdata_nu<-newdata
    newdata_nu$dev=as.numeric(as.character(newdata$dev))
    pred_mu<-predict(model,parameter="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,parameter="sigma",data=data,newdata=newdata,type="response")
...
fit_param_ZALN<-function(model,data,newdata){
    ...
    pred_mu<-predict(model,parameter="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,parameter="sigma",data=data,newdata=newdata,type="response")
    pred_nu<-mean(predict(model,parameter="xi0",data=data,newdata=newdata_nu,type="response"))
...
simulate_ZALN<-function(param){
    k=1
    simy<-replicate(k,rZALN(n=length(param$pred_mu),mu=param$pred_mu,sigma=param$pred_sigma,xi0=param$pred_nu))
    return(simy)
}
...
cal_dens_ZAGA<-function(y, model,data,newdata){
    newdata_nu<-newdata
    newdata_nu$dev=as.numeric(as.character(newdata$dev))
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
...
cal_mu_ZAGA<-function(model,data,newdata){
    newdata_nu<-newdata
    newdata_nu$dev=as.numeric(as.character(newdata$dev))
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_nu<-predict(model,what="nu",data=data,newdata=newdata_nu,type="response")
...
##Calculate ZAGA mean for ZAGA GLM
cal_sigma_ZAGA<-function(model,data,newdata){
    newdata_nu<-newdata
    newdata_nu$dev=as.numeric(as.character(newdata$dev))
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
    return(pred_sigma)
...
cal_CDF_ZAGA<-function(y, model,data,newdata, new_y = F){
    newdata_nu<-newdata
    newdata_nu$dev=as.numeric(as.character(newdata$dev))
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
...
fit_param_ZAGA<-function(model,data,newdata){
    ...
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
    pred_nu<-mean(predict(model,what="nu",data=data,newdata=newdata_nu,type="response"))
    return(list(pred_mu=pred_mu, pred_sigma=pred_sigma, pred_nu=pred_nu)) 
}

simulate_ZAGA<-function(param){
    k=1
    simy<-replicate(k, rZAGA(n=length(param$pred_mu),mu=param$pred_mu,sigma=param$pred_sigma,nu=param$pred_nu))
    return(simy)
}
...
cal_dens_PPCI<-function(y, model,N,newdata){
  N_rep<-vlookup_N_i(N,newdata)
  pred_ODP<-predict.glm(model,newdata=newdata,type="response",se.fit=TRUE)
  mu<-pred_ODP$fit*N_rep
  dens<-c()
  for (i in 1:nrow(newdata)){
    phi<-(pred_ODP$residual.scale)^2*(N_rep[i])^2
    dens[i]<-dODP(y=y[i],lambda=mu[i],phi=phi)
  }
  return(dens)
}

##Calculate ODP mean for PPCI model
cal_mu_PPCI<-function(model,N,newdata){
  N_rep<-vlookup_N_i(N,newdata)
  pred_ODP<-predict.glm(model,newdata=newdata,type="response",se.fit=TRUE)
  mu<-pred_ODP$fit*N_rep
  return(mu)
...
##Calculate ODP mean for PPCI model
cal_sigma_PPCI<-function(model,N,newdata){
  N_rep<-vlookup_N_i(N,newdata)
  pred_ODP<-predict.glm(model,newdata=newdata,type="response",se.fit=TRUE)
  mu<-pred_ODP$fit*N_rep
  sigma<-c()
  for (i in 1:nrow(newdata)){
    sigma[i]<-(pred_ODP$residual.scale)^2*(N_rep[i])^2*mu[i]
    
  }
  return(sigma)
}

cal_CDF_PPCI<-function(y, model,N,newdata, new_y = F){
  N_rep<-vlookup_N_i(N,newdata)
  pred_ODP<-predict.glm(model,newdata=newdata,type="response",se.fit=TRUE)
  mu<-pred_ODP$fit*N_rep
...
cal_dens_PPCF<-function(y, model_subCount,model_subPayments,N, data,newdata){
    ...
    dens_PPCF<-c()
    for (i in 1:nrow(newdata)){
      ds<-dODP(y=y[i],lambda=mu[i],phi=phi[i])
      dens_PPCF[i]<-ds
    }
...
##Calculate ODP mean for PPCF model
cal_sigma_PPCF<-function(model_subCount,model_subPayments, N, data,newdata){
    pred_F<-predict(model_subCount,newdata=newdata,type="response")
    pred_OT <- calc_pred_OT(model_subCount, N, data, newdata)
...
cal_dens_GA_Gamlss<-function(y, model,tau,data,newdata){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
    return(dGA(x=y+tau,mu=pred_mu,sigma=pred_sigma))
}
...
cal_mu_GA_Gamlss<-function(model,tau,data,newdata){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    return(pred_mu)
}

##Calculate GA mean for GA GAMLSS
cal_sigma_GA_Gamlss<-function(model,tau,data,newdata){
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
    return(pred_sigma)
}

##Calculate Gamma CDF for Gamma GAMLSS
cal_CDF_GA_Gamlss<-function(y, model,tau,data,newdata, new_y = F){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
    if (!new_y) {return(pGA(q=y,mu=pred_mu,sigma=pred_sigma))}
    else {return(mapply(
...
cal_dens_LN_Gamlss<-function(y, model,tau,data,newdata){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
    return(dLNO(x=y+tau,mu=pred_mu,sigma=pred_sigma))
}
...
cal_mu_LN_Gamlss<-function(model,tau,data,newdata){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
...
##Calculate LN mean for LN GAMLSS
cal_sigma_LN_Gamlss<-function(model,tau,data,newdata){
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
    return(pred_sigma)
}

##Calculate Log-Normal CDF for Log-Normal GAMLSS
cal_CDF_LN_Gamlss<-function(y, model,tau,data,newdata, new_y = F){
    pred_mu<-predict(model,what="mu",data=data,newdata=newdata,type="response")
    pred_sigma<-predict(model,what="sigma",data=data,newdata=newdata,type="response")
    if (!new_y) {return(pLNO(q=y,mu=pred_mu,sigma=pred_sigma))}
    else {return(mapply(
