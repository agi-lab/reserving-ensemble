...
This section generates the statistical test results for Log Score and CRPS as shown in **6.2.3. Analysing the difference in distributional forecast performance using statistical tests** in the paper. 
...
ggplot(data=stack(data.frame(mean_score_over_sim)[, c(optim_ADLP_index, optim_ADLP_index_CRPS, 20, 21)]), aes(x = ind, y = values)) + 
    ylim(12, 22) +
    geom_boxplot()+labs(x="Model",y="CRPS") +
    scale_x_discrete(labels=c(all_ensembles_names[optim_ADLP_index],all_ensembles_names[optim_ADLP_index_CRPS], 'EW', 'BMV'))  + ggtitle(paste("Average CRPS over 100 simulations"))
...
plot(x=corres_splitpoints_SLPincluded, y=CRPS_comparision_ADLP,
     ...
     main=paste("Mean CRPS v.s. Different split points"))
...
legend('bottomleft' ,legend=c("SLP",all_ensembles_names[optim_ADLP_index_CRPS], all_ensembles_names[optim_ADLP_index]),col=c("grey","black", "brown"),pch=c(16,16),cex=1)
...
```{r Calc CRPS DM and Adj. DM Test Scores}
# names(all_ensembles)
# EW ensemble is index 20
# BMV ensemble is index 21
...
```{r Calc CRPS DM and Adj. DM Test Statistics}
...
ggplot(data=stack(as.data.frame(TestStats_OptimADLP_ADLP)),aes(x=ind,y=values)) +
    geom_boxplot() +
    labs(x="Hypothesis tests",y="Test statistics") +
    ggtitle("Distribution of DM Statistics; CRPS (ADLP 8 v.s ADLP 12)") +
    geom_hline(yintercept=qnorm(0.975),linetype="dashed", color = "red", linewidth=2) +
    scale_x_discrete(labels=c('Optimal ADLP (Log Score) v.s Optimal ADLP (CRPS)'))
...
## Results for weighted CRPS calculation

This section calculates and presents the CRPS results of the fitted ensembles as shown in **Appendix D.4 Results for weighted CRPS calculation** in the paper.
...
```{r Calc Weighted CRPS DM and Adj. DM Test Scores}
## Calculate test statistics for weighted CRPS:
DM_WeightedCRPS_ADLPvsBMV <- calc_DM_test_models(test_crps, optim_ADLP_index, BMV_ind)
DM_WeightedCRPS_ADLPvsEW <- calc_DM_test_models(test_crps, optim_ADLP_index, EW_ind)
DM_WeightedCRPS_ADLPvsSLP <- calc_DM_test_models(test_crps, optim_ADLP_index, SLP_ind)
```

```{r Calc Weighted CRPS DM and Adj. DM Test Statistics}
print("Weighted CRPS DM test: ADLP vs BMV")
mean(-DM_WeightedCRPS_ADLPvsBMV > qnorm(0.975))
print("Weighted CRPS DM test: ADLP vs EW")
mean(-DM_WeightedCRPS_ADLPvsEW > qnorm(0.975))
```


```{r Graphical Results: Weighted CRPS}

## Statistical test results: CRPS 

#pdf(paste0("plotting/triangle_", tri.size, "_", n.sims, "_testStatisticsOptimADLPvsBMV_WeightedCRPS.pdf"))

TestStats_OptimADLP_BMV <- data.frame(
    DM_BMV_weighted=-DM_WeightedCRPS_ADLPvsBMV,
    DM_BMV = -DM_CRPS_ADLPvsBMV
)
...
ggplot(data=stack(as.data.frame(TestStats_OptimADLP_BMV)),aes(x=ind,y=values)) + 
    geom_boxplot() +
    labs(x="Hypothesis tests",y="Test statistics") +
    ggtitle("Distribution of DM Statistics; Weighted CRPS & CRPS (Optimal ADLP v.s BMV)") +
    geom_hline(yintercept=qnorm(0.975),linetype="dashed", color = "red", linewidth=2) +
    scale_x_discrete(labels=c('Optimal ADLP v.s BMV(Weighted CRPS)', 'Optimal ADLP v.s BMV(CRPS)'))

#dev.off()


#pdf(paste0("plotting/triangle_", tri.size, "_", n.sims, "_testStatisticsOptimADLPvsEW_WeightedCRPS.pdf"))

TestStats_OptimADLP_EW <- data.frame(
    DM_EW=-DM_WeightedCRPS_ADLPvsEW
)

ggplot(data=stack(as.data.frame(TestStats_OptimADLP_EW)),aes(x=ind,y=values)) +
    geom_boxplot() +
    labs(x="Hypothesis tests",y="Test statistics") +
    ggtitle("Distribution of DM Statistics; Weighted CRPS (Optimal ADLP v.s EW)") +
    geom_hline(yintercept=qnorm(0.975),linetype="dashed", color = "red", linewidth=2) +
    scale_x_discrete(labels=c('Optimal ADLP v.s EW(DM)'))
...
TestStats_OptimADLP_SLP <- data.frame(
    DM_SLP=-DM_WeightedCRPS_ADLPvsSLP
)
