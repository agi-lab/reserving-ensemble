...
## Graphical Results of GoF Measures
...
ggplot(data=stack(as.data.frame(TestStats_p_val_SLP)),aes(x=ind,y=values))+geom_boxplot()+labs(x="Hypothesis tests",y="p-values")+ggtitle("Distribution of Log-score p-values (SLP v.s EW & BMV)")+geom_hline(yintercept=0.05,linetype="dashed", color = "red", size=1)+scale_x_discrete(labels=c('SLP v.s EW(DM)','SLP v.s BMV(DM)'))+geom_hline(yintercept=0.1,linetype="dashed", color = "blue", size=1)
#dev.off()


#pdf(paste0("simulation/triangle_", tri.size, "-data/TestStats_p_val_ADLP.pdf"))

ggplot(data=stack(as.data.frame(TestStats_p_val_ADLP)),aes(x=ind,y=values))+geom_boxplot()+labs(x="Hypothesis tests",y="p-values")+ggtitle("Distribution of Log-score p-values (ADLP v.s EW, BMV & SLP)")+geom_hline(yintercept=0.05,linetype="dashed", color = "red", size=1)+scale_x_discrete(labels=c('ADLP v.s EW(DM)','ADLP v.s BMV(DM)', 'ADLP v.s SLP(DM)'))+geom_hline(yintercept=0.1,linetype="dashed", color = "blue", size=1)
...
ggplot(data=stack(as.data.frame(TestStats_OptimADLP_BMV)),aes(x=ind,y=values)) + 
    geom_boxplot() +
    labs(x="Hypothesis tests",y="Test statistics") +
    ggtitle("Distribution of DM Statistics (Optimal ADLP v.s BMV)") +
    geom_hline(yintercept=qnorm(0.975),linetype="dashed", color = "red", size=2) +
...
ggplot(data=stack(as.data.frame(TestStats_OptimADLP_EW)),aes(x=ind,y=values)) +
    geom_boxplot() +
    labs(x="Hypothesis tests",y="Test statistics") +
    ggtitle("Distribution of DM Statistics (Optimal ADLP v.s EW)") +
    geom_hline(yintercept=qnorm(0.975),linetype="dashed", color = "red", size=2) +
...
ggplot(data=stack(as.data.frame(TestStats_OptimADLP_ADLP)),aes(x=ind,y=values)) +
    geom_boxplot() +
    labs(x="Hypothesis tests",y="Test statistics") +
    ggtitle("Distribution of CRPS DM Statistics (ADLP 12 v.s ADLP 13)") +
    geom_hline(yintercept=qnorm(0.975),linetype="dashed", color = "red", size=2) +
    scale_x_discrete(labels=c('ADLP 12 v.s ADLP 13(DM)'))
...
ggplot(data=stack(as.data.frame(TestStats_p_val_ADLP_CRPS)),aes(x=ind,y=values))+geom_boxplot()+labs(x="Hypothesis tests",y="p-values")+ggtitle("Distribution of CPRS p-values (ADLP v.s EW, BMV and SLP)")+geom_hline(yintercept=0.05,linetype="dashed", color = "red", size=1)+scale_x_discrete(labels=c('ADLP v.s EW(DM)','ADLP v.s BMV(DM)', 'ADLP v.s SLP(DM)', 'ADLP 12 v.s ADLP 13'))+geom_hline(yintercept=0.1,linetype="dashed", color = "blue", size=1)
...
## Calculating Aggregate Reserve Bias
...
```{r}
AME_75quantile <- apply((quantile_reserve_estimate - quantile_reserve)/quantile_reserve_estimate, MARGIN = 2, FUN = function(x) abs(median(x)))
plot(x=corres_splitpoints_SLPincluded, y = AME_75quantile[-c(EW_ind, BMV_ind)], xlab = "Accident Periods", ylab = "Absolute Median error", ylim = c(0, mean_reserve_bias[EW_ind]), main = "Absolute median error (75 quantile)")
abline(h = mean_reserve_bias[EW_ind], col = 'red')
abline(h = mean_reserve_bias[BMV_ind], col = 'green')

#AME_central <- apply(relative_diff_centralBias, MARGIN = 2, FUN = function(x) abs(median(x)))
...
# Investigation over Estimated Model Weights
