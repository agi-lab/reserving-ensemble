...
simulate_reserves <- function(n.sims, sims, component_models, ensemble_models) {
    ...
    for (s in 1:n.sims) {
        ...
        for (j in 1:n.ensembles){
            ...
            all_reserve_simulations[[j]][, s] <- apply(individual_claim_simulations,FUN=function(x) sum(x*10000),MARGIN=2)
        }
    }
...
predict_component_partitions <- function(component_models, s, data, newdata) {
    tau_Ga <- 5
    tau_LN <- 5
